...
class ReceiptLabeler:
    ...
    # Fungsi format angka ribuan (contoh 123456 -> 123.456)
    def format_number_entry(self, entry):
        val = entry.get().replace(".", "").strip()
        if not val:
            return
        try:
            num = int(val)  # ini otomatis handle minus
            entry.delete(0, tk.END)
            entry.insert(0, f"{num:,}".replace(",", "."))
        except ValueError:
            pass
    ...
    def calculate_total(self):
        ...
        # Update total_entry (readonly, jadi harus ubah state dulu)
